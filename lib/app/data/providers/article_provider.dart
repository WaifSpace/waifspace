...
class ArticleProvider {
  ...
  Future<List<Article>> latestArticles(Article? article) async {
    ...
    var sqlBuilder = SqlBuilder(
        "SELECT a.*, b.name as source_name, b.homepage as homepage FROM $table as a left join ${ArticleSourceProvider.table} as b on a.source_id = b.id");
    sqlBuilder.limit(_queryLimit).where("a.source_id = ?", [
      _sourceIDCondition
    ]).like(
        '(a.title like ? or a.cn_title like ? or a.content like ? or a.cn_content like ?)',
        [
          _searchCondition,
          _searchCondition,
          _searchCondition,
  ...
  Future<void> create(Article article) async {
    ...
    if (maps.isEmpty) {
      ...
      if (!isChinese(article.title ?? '') &&
          !article.sourceName!.toLowerCase().contains("github")) {
        // 只对英文处理, github的标题不处理
        article.cnTitle = await AIService.to.translate(article.title ?? '');
      }

      // 在保存进数据库的时候，调用chatgpt 提取文章的内容, 因为现在 gpt 便宜了，所以可以用来做摘要
      article.cnContent =
          await AIService.to.readAndTranslate(article.content ?? "");
  ...
  Future<int> readArticle(int articleID) async {
    ...
    return await _db
        .rawUpdate('update $table set is_read = 1 where id = ?', [articleID]);
  }

  Future<Map> sourceCount() async {
    var countInfo = await _db.rawQuery(
        'select source_id, count(id) as count from $table group by source_id');
    return {for (var e in countInfo) e['source_id']: e['count']};
  }

  Future<Map> sourceUnreadCount() async {
    var countInfo = await _db.rawQuery(
        'select source_id, count(id) as count from $table where is_read = 0 group by source_id');
    return {for (var e in countInfo) e['source_id']: e['count']};
  }

  Future<String> source24HoursCount() async {
    var countInfo = await _db.rawQuery(
        'select count(id) as count from $table where pub_date > ?',
        [AppTime.fromNow(24).dbFormat()]);
    return countInfo.first['count'].toString();
  }

  Future<String> source24HoursUnreadCount() async {
    var countInfo = await _db.rawQuery(
        'select count(id) as count from $table where is_read = 0 and pub_date > ?',
        [AppTime.fromNow(24).dbFormat()]);
    return countInfo.first['count'].toString();
  }
  ...
  Future<int> makeAllRead() async {
    logger.i('标记所有文章已读');
    return await _db
        .rawUpdate('update $table set is_read = 1 where is_read = 0');
  }

  Future<int> make24HoursRead() async {
    logger.i('标记24小时文章已读');
    return await _db.rawUpdate(
        'update $table set is_read = 1 where is_read = 0 and pub_date > ?',
        [AppTime.fromNow(24).dbFormat()]);
  }
  ...
  Future<int> deleteBySourceID(int sourceID) async {
    ...
    return await _db
        .rawDelete('delete from $table where source_id = ?', [sourceID]);
  }
